...
for x = 1:fil
    for y = 1:col
        Y_mean = Y_mean + img_YCbCr(x,y,1);
        Cb_mean = Cb_mean + img_YCbCr(x,y,1);
        Cr_mean = Cr_mean + img_YCbCr(x,y,1);
    end
end

Y_mean = uint8(Y_mean/(fil*col));
Cb_mean = uint8(Cb_mean/(fil*col));
Cr_mean = uint8(Cr_mean/(fil*col)); 


for x = 1:fil
    for y = 1:col
         if(img_YCbCr(x,y,1)>=Y_mean) && (img_YCbCr(x,y,2)<=Cb_mean) && (img_YCbCr(x,y,3)>=Cr_mean)
